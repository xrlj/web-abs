...
@Component({
  selector: 'app-product-annex-type',
  templateUrl: './product-annex-type.component.html',
  styleUrls: ['./product-annex-type.component.less']
})
export class ProductAnnexTypeComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private productAnnexTypeService: ProductAnnexTypeService,
    ...
    this.addOrEditForm = this.fb.group({
      ...
      annexTypeSort: [1, [MyValidators.required]],
  ...
  search(resetPageIndex: boolean = false) {
    ...
    this.productAnnexTypeService.getListPage(body)
      .ok(data => {
  ...
  del(id: string) {
    this.uiHelper.modalDel('确定删除?')
      .ok(() => {
        ...
        this.productAnnexTypeService.delete(id)
          .ok(data => {
            setTimeout(() => {
              this.search(false);
            }, 100);
  ...
  handleOk() {
    if (this.addOrEditForm.valid) {
      ...
      this.productAnnexTypeService.addOrUpdate(value)
        .ok(data => {
          this.resetAddOrEditModal();
          setTimeout(() => {
            this.search(false);
  ...
  edit(id) {
    this.defaultBusService.showLoading(true);
    this.productAnnexTypeService.get(id)
      .ok(data => {
        this.details = data;
        this.dialogType = 2;
        this.isShowModal = true;
